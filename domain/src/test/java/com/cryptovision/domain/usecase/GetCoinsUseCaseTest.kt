...
/**
 * Unit tests for GetCoinsUseCase.
 * Demonstrates testing best practices with JUnit5, MockK, and Turbine.
 */
class GetCoinsUseCaseTest {
    ...
    fun `when repository returns success, use case should emit success`() = runTest {
        ...
        val expectedCoins = listOf(
            createTestCoin(id = "bitcoin", name = "Bitcoin"),
            createTestCoin(id = "ethereum", name = "Ethereum")
        )
        coEvery { repository.getCoins(any()) } returns flowOf(Result.Success(expectedCoins))
        ...
        result.test {
            ...
            assertEquals(expectedCoins, (emission as Result.Success).data)
            awaitComplete()
        }
    }
    ...
    fun `when repository returns error, use case should emit error`() = runTest {
        ...
        coEvery { repository.getCoins(any()) } returns flowOf(Result.Error(expectedException))
    ...
    fun `when repository emits loading then success, use case should emit both`() = runTest {
        // Given
        val expectedCoins = listOf(createTestCoin(id = "bitcoin", name = "Bitcoin"))
        coEvery { repository.getCoins(any()) } returns flowOf(
            Result.Loading,
            Result.Success(expectedCoins)
        )
        ...
        result.test {
            ...
            assertEquals(expectedCoins, (successEmission as Result.Success).data)
            awaitComplete()
        }
    }

    @Test
    fun `when forceRefresh is true, should pass it to repository`() = runTest {
        // Given
        val expectedCoins = listOf(createTestCoin(id = "bitcoin", name = "Bitcoin"))
        coEvery { repository.getCoins(forceRefresh = true) } returns flowOf(Result.Success(expectedCoins))
    ...
    @Test
    fun `when repository returns empty list, use case should emit empty list`() = runTest {
        // Given
        coEvery { repository.getCoins(any()) } returns flowOf(Result.Success(emptyList()))
        ...
        result.test {
            val emission = awaitItem()
            assertTrue(emission is Result.Success)
            assertTrue((emission as Result.Success).data.isEmpty())
            awaitComplete()
        }
    }

    // Helper function to create test coin
