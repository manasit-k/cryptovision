...
class GetFavoriteCoinsUseCaseTest {
    ...
    @Test
    fun `when repository returns success with favorites, use case should emit favorites list`() = runTest {
        // Given
        val expectedCoins = listOf(
            createTestCoin(id = "bitcoin", name = "Bitcoin"),
            createTestCoin(id = "ethereum", name = "Ethereum")
        )
        coEvery { repository.getFavoriteCoins() } returns flowOf(Result.Success(expectedCoins))
    ...
    fun `when repository returns empty list, use case should emit empty list`() = runTest {
        // Given
        coEvery { repository.getFavoriteCoins() } returns flowOf(Result.Success(emptyList()))
    ...
    fun `when repository returns error, use case should emit error`() = runTest {
        ...
        coEvery { repository.getFavoriteCoins() } returns flowOf(Result.Error(expectedException))
    ...
    fun `when repository emits loading then success, use case should emit both`() = runTest {
        ...
        coEvery { repository.getFavoriteCoins() } returns flowOf(
            Result.Loading,
